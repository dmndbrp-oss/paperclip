...
- **Author ≠ reviewer, structurally.** PR author = the executor's GitHub identity (`dmndbrp-oss`). PR reviewer/merger = the Director of Engineering acting under the `sage-merge-bot` identity — a distinct agent AND a distinct GitHub account from the author. No agent approves or merges its own PR.
- **Auto-mergeable by the DoE (no board escalation needed) only when ALL of the following hold:**
  ...
  3. The approving GitHub review is submitted by `sage-merge-bot` (≠ author).
